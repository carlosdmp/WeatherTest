...
import com.cdmp.weatherapp.databinding.FragmentMainBinding
import com.cdmp.weatherapp.domain.model.Point
import com.cdmp.weatherapp.domain.model.WeatherRequest
import com.cdmp.weatherapp.domain.model.WeatherRequest.CityRequest
import com.cdmp.weatherapp.domain.model.WeatherRequest.ZipRequest
import com.cdmp.weatherapp.presentation.model.*
...
class MainFragment : Fragment() {
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        with(binding) {
            setupInput(etNameInput, ibNameSearch) { CityRequest(it) }
            setupInput(etZipInput, ibZipSearch) { ZipRequest(it) }
    ...
    private fun showError(error: ErrorDisplay) {
        ...
        context?.let { context ->
            ...
            currentDialog = AlertDialog.Builder(context)
                .setTitle(getString(com.cdmp.weatherapp.R.string.error_dialog_header))
                .setMessage(
                    when (error) {
                        NotFoundErrorDisplay -> getString(com.cdmp.weatherapp.R.string.not_found_error_message)
                        BadRequestErrorDisplay -> getString(com.cdmp.weatherapp.R.string.bad_input_error_message)
                        is UnknownErrorDisplay -> error.message
                            ?: getString(com.cdmp.weatherapp.R.string.unknown_error_message)
                    }
                )
